...
Plain text only — no emojis. Hierarchy comes from bold headers and spacing.
"""
...
def _streak(h: dict) -> str:
    n = h.get("streak")
    return f" ({n}-day streak)" if n else ""
...
def format_morning(tasks: list, events: list, today: date, habits: list = None) -> str:
    ...
    if high_priority:
        lines.append("<b>High Priority — do regardless of energy</b>")
    ...
    if due_habits:
        ...
        lines += [f"• {_esc(h['name'])}{_streak(h)}" for h in due_habits]

    stale = [t for t in tasks if t.get("stale")]
    if stale:
        lines += ["", "<b>Stale — waiting too long</b>"]
...
def format_midday(open_tasks: list, done_count: int, afternoon_events: list, today: date) -> str:
    ...
    for t in top:
        lines.append(f"• {_esc(t.get('name',''))}")

    due = [t for t in open_tasks if _due_today_or_past(t, today_iso)]
    if due:
        lines += [""] + [f"Due today: {_esc(t['name'])}" for t in due]
    return "\n".join(lines)
...
def format_eod(done_today: list, rolling: list, tomorrow_events: list, today: date, habits: list = None) -> str:
    ...
    if missed_habits:
        ...
        lines += [f"• {_esc(h['name'])}{_streak(h)}" for h in missed_habits]
    ...
    if past_due:
        lines += [""] + [f"Past due: {_esc(t['name'])}" for t in past_due]

    stale = [t for t in rolling if t.get("stale")]
    if stale:
        lines += ["", f"{len(stale)} task(s) stale 3+ days — timebox one tomorrow or drop it."]
...
def format_habits_status(habits: list) -> str:
    ...
    if due_open:
        ...
        lines += [f"• {_esc(h['name'])}{_streak(h)}" for h in due_open]
    if done:
        lines += ["", "<b>Done today</b>"]
        lines += [f"• {_esc(h['name'])}{_streak(h)}" for h in done]
